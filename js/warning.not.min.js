...
registerPaint('warning', class {
    ...
    paint(ctx, geom, props) {
      let color = props.get('--warning-color') || 'yellow';
      const schemes = {
        'yellow': [
          '#ffc821',
          '#faf100',
      ...
      color = color.toString().replace(/\s/, '');
      color = Object.keys(schemes).indexOf(color) >= 0 ? color : 'yellow';
      const ratio = 125 / 105;
      ...
      const lineWidth = 8;
      const innerBorder = 5;
      const primaryColor = schemes[color][0];
      const secondaryColor = schemes[color][1];
      const tertiaryColor = schemes[color][2];
      ...
      reflection.addColorStop(.5, tertiaryColor);
      reflection.addColorStop(1, secondaryColor);
